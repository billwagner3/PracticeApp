...
export class KeyPickComponent implements OnInit {
  ...
  getRandomKey(){
    ...
    let typeKey = randomKey['randomKey.name']
    console.log(this.tyepKey);
    return randomKey;
    }

    getKeyToDrill() {
      let typeKey = this.getRandomKey()
    }
  ...
  submit() {
    ...
    .map((v, i) => v ? this.keyNames[i].name : null)
    .filter(v => v !== null);
    let firstKey = this.getRandomKey();
    console.log(keysPicked, firstKey);    
  }
