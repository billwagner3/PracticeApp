...
export class KeyPickComponent implements OnInit {
  ...
  submit() {
    ...
    let drillKey = firstKey.name;
    console.log(drillKey);
    if (randomKey != null) {
      this.dataPassed = true;
      this.errors = false;   
    }
    console.log(keysPicked, drillKey); 
    let x = document.getElementById("drillKey").innerHTML = "Key to Drill: " + drillKey;
    return x;
  ...
  //   let firstKey = keysPicked.length;
  //   let randomKey = Math.floor(Math.random() * firstKey);
  //   var drillKey = this.keyNames[randomKey].name;
  //   if (randomKey != null) {
  //     this.dataPassed = true;
  //     this.errors = false;   
  // }
...
// }
  ...
  check = this.keysService.getScale(this.tyepKey)
  
  checkAnswer(check) {
    if (this.submit() === check)
      {
      console.log(check);
